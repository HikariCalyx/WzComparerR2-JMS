...
namespace WzComparerR2.WzLib.Compatibility
{
    ...
    public sealed class Pkg2HashVersionCalc64V2 : IPkg2HashVersionCalc<ulong>
    {
        ...
        public bool Verify(ulong hash1, ulong hash2, ulong hashVersion)
        {
            // v42 = sub_1529FA450(hash_ver, hash1)
            ulong v42 = Sub1529FA450(hashVersion, hash1);

            // v43 = __ROR8__(0xBF58476D1CE4E5B9 * (v42 ^ 0x84CAA73B2BB70682 ^ ((v42 ^ 0x84CAA73B2BB70682) >> 30)), 27)
            ulong t = v42 ^ 0x84CAA73B2BB70682UL;
            ulong v43 = ROR8(0xBF58476D1CE4E5B9UL * (t ^ (t >> 30)), 27);

            // hash2_calc = (0x94D049BB133111EB * (v43 ^ (v43 >> 27))) ^ ((...) >> 31)
            ulong hx = 0x94D049BB133111EBUL * (v43 ^ (v43 >> 27));
            ulong hash2_calc = hx ^ (hx >> 31);

            return hash2_calc == hash2;
        }
    }
