...
namespace WzComparerR2.CharaSimControl
{
    public class GearTooltipRender2 : TooltipRender
    {
        ...
        private Bitmap RenderBase(out int picH)
        {
            ...
            {
                TextRenderer.DrawText(g, "LIMITEDラベル", GearGraphics.EquipMDMoris9Font, new Point(261, picH), Color.FromArgb(248, 196, 129), TextFormatFlags.HorizontalCenter);
                picH += 16;
            ...
            if ((Gear.type == GearType.face_n || Gear.type == GearType.hair_n || Gear.type == GearType.head_n || Gear.type == GearType.hair2_n) && Gear.Props.TryGetValue(GearPropType.cosmetic, out value) && value >= 0)
            {
                ...
                if (Gear.type != GearType.head_n) GearGraphics.DrawString(g, $"色：#c{colorName}#", GearGraphics.EquipDetailFont, orange2FontColorTable, 13, 244, ref picH, 15);
                TextRenderer.DrawText(g, "外見：", GearGraphics.EquipDetailFont, new Point(13, picH), Color.White, TextFormatFlags.NoPadding);
                if (this.avatar == null)
                {
                    this.avatar = new AvatarCanvasManager();
            ...
            if (CharaSimLoader.LoadedPetEquipInfo.ContainsKey(Gear.ItemID))
            {
                if (CharaSimLoader.LoadedPetEquipInfo[Gear.ItemID].Count > 2)
                {
                    ...
                    using (Graphics g = Graphics.FromImage(applicablePetBitmap))
                    {
                        ...
                        TextRenderer.DrawText(g, "ペットリスト", GearGraphics.ItemDetailFont, new Point(width, picH), Color.FromArgb(204, 255, 0), TextFormatFlags.HorizontalCenter);
                        int iconLeft = 15;
                        int txtLeft = 55;
                        for (int index = 0; index < reqColumns; index++)
                        {
