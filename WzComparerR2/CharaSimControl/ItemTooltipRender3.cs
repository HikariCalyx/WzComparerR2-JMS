    ...
    public class ItemTooltipRender3 : TooltipRender
    {
        ...
        private Bitmap RenderItem(out int picH, out List<int> splitterH)
        {
            ...
            if (item.Recipes.Count > 0)
            {
                ...
                GearGraphics.DrawString(g, $"要求レベル : {sr2.Name} Lv.{reqSkillLevel}", GearGraphics.ItemDetailFont, item22ColorTable, descLeft, descRight, ref picH, LineHeight);
                picH += 3;
            }
            else if (item.Props.TryGetValue(ItemPropType.reqLevel, out value) && value > 0)
            {
                GearGraphics.DrawString(g, $"要求レベル : Lv.{value}", GearGraphics.ItemDetailFont, item22ColorTable, descLeft, descRight, ref picH, LineHeight);
                picH += 3;
            }
            ...
            {
                GearGraphics.DrawString(g, $"#c装着時1回に限り{incline.Substring(2)}の経験値を獲得できます。(1日獲得期間の最大値を超えると、獲得できません)#", GearGraphics.ItemDetailFont, item22ColorTable, descLeft, descRight, ref picH, LineHeight);
                picH += 3;
            }
            ...
            {
                GearGraphics.DrawString(g, "( 制作可能回数：" + value + "回)", GearGraphics.ItemDetailFont, item22ColorTable, descLeft, descRight, ref picH, LineHeight);
                picH += 3;
            }
            else if (item.Specs.TryGetValue(ItemSpecType.recipeValidDay, out value) && value > 0)
            {
                GearGraphics.DrawString(g, "( 制作可能期間：" + value + "日)", GearGraphics.ItemDetailFont, item22ColorTable, descLeft, descRight, ref picH, LineHeight);
                picH += 3;
            }
            ...
            if (item.IsPet)
            {
                ...
                {
                    GearGraphics.DrawString(g, "#c[使用可能な命令語]#", GearGraphics.ItemDetailFont, item22ColorTable, descLeft, descRight, ref picH, LineHeight);
        ...
        private List<string> GetItemBottomAttributeString(StringResult sr, bool isTranslateRequired = false)
        {
            ...
            if (item.Cash)
            {
                ...
                else if ((!item.Props.TryGetValue(ItemPropType.tradeBlock, out value) || value == 0))
                {
                    ...
                    {
                        tags.Add("#cダブルクリックすると、プレビューでボックス内のアイテムを3秒ごとに順番に確認できます。#\\n#$rキャッシュライブラリでダブルクリックして使用できます。ボックスは交換できません。\\nNEXONポイントで購入すると、使用前1回に限り、箱から獲得した報酬は他人と交換することができます。\r\n#");
                    }
                    else if (!(item.ItemID / 10000 == 501 || item.ItemID / 10000 == 502 || item.ItemID / 10000 == 516))
                    {
                        tags.Add("#$rNEXONポイントで購入時に使用前に他人と1回交換可能#");
                    }
                }
            }
            ...
            if (item.Props.TryGetValue(ItemPropType.tradeAvailable, out value) && value > 0)
            {
                switch (value)
                {
                    case 1: tags.Add("#$rカルマのはさみを使用すると1回交換可能#"); break;
                    case 2: tags.Add("#$rカルマのはさみを使用すると1回交換可能#"); break;
                }
            }
        ...
        private Bitmap RenderSummoningSackMobs()
        {
            if (Item.SummoningSack.Count <= 3)
            {
                ...
                foreach (var i in Item.SummoningSack)
                {
                    ...
                    if (mobNode != null)
                    {
                        ...
                        {
                            int picH = 2;
                            GearGraphics.DrawPlainText(g, maxCount == minCount ? $"{maxCount}体" : $"{minCount}～{maxCount}体", mobTypeFont, Color.FromArgb(255, 255, 255), 2, 64, ref picH, 10);
                        }
            ...
            else
            {
                ...
                foreach (var i in Item.SummoningSack)
                {
                    int maxCount = i.Rate.Count;
                    int minCount = i.Rate.Count(n => n == 100);
                    string quantity = maxCount == minCount ? $"{maxCount}体" : $"{minCount}～{maxCount}体";
                    StringResult sr;
                ...
                using (Graphics g = Graphics.FromImage(result))
                {
                    GearGraphics.DrawNewTooltipBack(g, 0, 0, width, picHeight);
                    int picH = 8; 
                    TextRenderer.DrawText(g, "包みの中のモンスター一覧", GearGraphics.ItemDetailFont, new Point(width, picH), Color.FromArgb(204, 255, 0), TextFormatFlags.HorizontalCenter);
                    picH = 25;
                    foreach (var i in mobDict.Keys)
                    {
