    ...
    public class QuestTooltipRenderer : TooltipRender
    {
        ...
        private Bitmap RenderBase()
        {
            ...
            if (this.Quest.Lvmin > 0)
            {
                ...
                {
                    TextRenderer.DrawText(g, $"推奨レベル: {this.Quest.Lvmin}以上", GearGraphics.EquipMDMoris9Font, new Point(left, 90), ((SolidBrush)GearGraphics.QuestBrushDefault).Color, TextFormatFlags.NoPadding);
                }
                else
                {
                    var lvMax = this.Quest.Lvmax > 0 ? this.Quest.Lvmax : this.Quest.Lvmin + 4;
                    TextRenderer.DrawText(g, $"推奨レベル: {this.Quest.Lvmin}～{lvMax}", GearGraphics.EquipMDMoris9Font, new Point(left, 90), ((SolidBrush)GearGraphics.QuestBrushDefault).Color, TextFormatFlags.NoPadding);
                }
            }
            if (npcImage.Bitmap != null)
            {
            ...
            TextRenderer.DrawText(g2, $"状態: {stateText[state]}" + (this.Quest.Blocked ? " / クエスト開始不可" : ""), GearGraphics.EquipMDMoris9Font, new Point(21, picH + Margin_top - 26), ((SolidBrush)GearGraphics.QuestBrushDefault).Color, TextFormatFlags.NoPadding);
